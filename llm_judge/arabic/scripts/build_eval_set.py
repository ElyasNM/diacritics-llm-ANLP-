"""
Build the full-scale Arabic LLM-judge evaluation set.

Targets:
  ARCD:   1,395 (all, after filtering)
  TyDiQA: 2,000 (stratified sample)
  MKQA:   1,000 (stratified sample)
  Total:  ~4,388 unique questions
  Calls:  ~4,388 x 6 = ~26,328

Usage:
  python build_eval_set.py \\
      --results_dir /path/to/ANLP-results \\
      --output_dir  ./full_eval
"""
...
def answer_length_bucket(answers):
    if not answers:
        return "short"
    min_len = min(len(a.split()) for a in answers if a.strip())
...
def build(args):
    ...
    canonical, skip_c, skip_p = load_and_filter_canonical(
        results_dir, "qwen", "no_tashkeel"
    )
    print(f"Canonical source (qwen/no_tashkeel):")
    print(f"  Total: {len(canonical)+skip_c+skip_p}  corrupted: {skip_c}  empty: {skip_p}  clean: {len(canonical)}")
    ...
    print(f"Estimated runtime: ~{n_judge * 3.24 / 4 / 3600:.1f} hrs with 4 workers "
          f"(based on observed 3.24s/call)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir",  default="full_eval")
