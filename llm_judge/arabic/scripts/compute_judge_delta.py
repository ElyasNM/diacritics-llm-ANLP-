"""
Compute Delta LLM-Judge = Judge_Accuracy(tashkeel) - Judge_Accuracy(no_tashkeel)

Since questions are perfectly paired, also computes per-question transitions:
  Plain CORRECT   -> Tashkeel CORRECT    (stable correct)
  Plain CORRECT   -> Tashkeel INCORRECT  (harmed by tashkeel)
  Plain INCORRECT -> Tashkeel CORRECT    (helped by tashkeel)
  Plain INCORRECT -> Tashkeel INCORRECT  (stable incorrect)

Reports:
  - Per model x dataset
  - Per model overall
  - Combined Arabic overall (all models + datasets)
...
Usage:
  python compute_judge_delta.py \\
      ...
      --report  full_eval/judge_delta_report.json
"""
...
def compute_transitions(plain_labels_by_id, tash_labels_by_id):
    """
    Compute the four transition counts for paired examples.
    Returns dict with keys: CC, CI, IC, II and totals.
    """
    common_ids = set(plain_labels_by_id.keys()) & set(tash_labels_by_id.keys())
    CC = CI = IC = II = 0
    for eid in common_ids:
        p = plain_labels_by_id[eid]
        t = tash_labels_by_id[eid]
...
def compute_delta(args):
    ...
    def fmt_row(model, label, n, plain_acc, tash_acc, delta):
        return (f"{model:<12} {label:<12} {n:>6}  "
                f"{plain_acc:>9.1f}% {tash_acc:>9.1f}% {delta:>+8.1f}%")
    ...
    for model in MODELS:
        for src in DATASETS:
            plain_d = idx.get((model, "no_tashkeel",  src), {})
            tash_d  = idx.get((model, "with_tashkeel", src), {})
            plain_acc, pn = accuracy(list(plain_d.values()))
            tash_acc,  tn = accuracy(list(tash_d.values()))
            if plain_acc is None or tash_acc is None:
                continue
            delta = round(tash_acc - plain_acc, 2)
            n = min(pn, tn)
            trans = compute_transitions(plain_d, tash_d)
            print(fmt_row(model, src, n, plain_acc, tash_acc, delta))
            report["per_model_dataset"][f"{model}_{src}"] = {
                "n": n, "plain_acc": plain_acc, "tash_acc": tash_acc,
    ...
    for model in MODELS:
        plain_d = idx_mc.get((model, "no_tashkeel"),  {})
        tash_d  = idx_mc.get((model, "with_tashkeel"), {})
        plain_acc, pn = accuracy(list(plain_d.values()))
        tash_acc,  tn = accuracy(list(tash_d.values()))
        if plain_acc is None or tash_acc is None:
            continue
        ...
        n = min(pn, tn)
        trans = compute_transitions(plain_d, tash_d)
        print(fmt_row(model, "OVERALL", n, plain_acc, tash_acc, delta))
        report["per_model_overall"][model] = {
            "n": n, "plain_acc": plain_acc, "tash_acc": tash_acc,
            "delta": delta, "transitions": trans,
        }

    # ── Combined Arabic overall ───────────────────────────────────────────────
    print("\n" + "=" * 75)
    print("DELTA REPORT — combined Arabic overall (all models + datasets)")
    print("=" * 75)

    all_plain = [r["judge_label"] for r in rows if r["condition"] == "no_tashkeel"]
    all_tash  = [r["judge_label"] for r in rows if r["condition"] == "with_tashkeel"]
    plain_acc, pn = accuracy(all_plain)
    tash_acc,  tn = accuracy(all_tash)
    delta = round(tash_acc - plain_acc, 2)
    print(fmt_row("ALL MODELS", "ALL DATA", min(pn, tn), plain_acc, tash_acc, delta))
    report["combined_overall"] = {
        "n": min(pn, tn), "plain_acc": plain_acc,
        "tash_acc": tash_acc, "delta": delta,
    }
    ...
    for model in MODELS:
        for bucket in BUCKETS:
            plain_d = idx_mcb.get((model, "no_tashkeel",  bucket), {})
            tash_d  = idx_mcb.get((model, "with_tashkeel", bucket), {})
            plain_acc, pn = accuracy(list(plain_d.values()))
            tash_acc,  tn = accuracy(list(tash_d.values()))
            if plain_acc is None or tash_acc is None:
                continue
            delta = round(tash_acc - plain_acc, 2)
            n = min(pn, tn)
            print(fmt_row(model, bucket, n, plain_acc, tash_acc, delta))
            report["per_bucket"][f"{model}_{bucket}"] = {
                "n": n, "plain_acc": plain_acc, "tash_acc": tash_acc, "delta": delta,
            }
        print()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report", default="full_eval/judge_delta_report.json")
