...
PER_CELL   = 7
F1_LEVEL_TARGETS = {"high_f1": 3, "medium_f1": 1, "low_f1": 3}
...
def save_outputs(sample, output_dir):
    ...
    for ex in sample:
        judge_input.append({
            ...
            "f1":         ex["f1"],
            "em":         ex["em"],
            "question":   ex["question"],   # NOTE: for with_niqud rows this will be vocalized;
                                    # use the fixed pilot files provided separately.
            "context":    ex["context"],
            "answers":    ex["answers"],
            "prediction": ex["prediction"],
        })
